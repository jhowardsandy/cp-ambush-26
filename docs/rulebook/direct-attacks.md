...
The timeline draws one seeded 1–100 accuracy roll for the legal area attempt and applies that same hit/miss outcome to every affected unit. A hit uses the ordinary per-target cover and armor calculation; a miss damages none. The attack may legally land on an empty tile and still consumes its configured ammunition once. Each affected unit receives its own `AttackResolved` event containing the target tile, radius, target result, and calculation breakdown.
