...
4. If it cannot attack from its current tile, it first considers a one-tile move followed by an attack on that same observed target. The move plus attack must fit the unit's action-point budget and the attack must be legal from the destination. It prefers cover, then shorter distance, then North, East, South, West. The attack is scheduled after that tile's movement ticks.
5. If no such sequence is legal, it chooses an unreserved legal adjacent tile that reduces its distance to the target. If several such tiles are equally near, it chooses the tile with the greatest cover value, then uses North, East, South, West as its final deterministic tie-break.
6. If no opposing unit is observable, it may take one legal step toward an authored, known scouting objective. If no objective is supplied, it uses the map center as a neutral fallback. It does not name or chase a hidden target.
...
Player-facing explanation: The event/decision log identifies whether the unit chose `heal`, `attack`, `move-attack`, `move`, `reposition`, `hold`, `scout`, or `wait`, and names the reason. A `move-attack` is two separately queued actions, not a special combat shortcut. The final encounter result still comes solely from the shared resolver, so its intended attack can fail if the board changes before its completion tick.

Not yet included: enemy use of contact memory/fog-of-war, posture tactics, overwatch policy, longer or branching multi-action AI plans, player-selected doctrines, stochastic choice, and difficulty tuning.

Tests/fixtures: `Pve_planner_is_deterministic_and_uses_the_same_attack_command_contract`, `Pve_planner_moves_toward_nearest_target_with_an_explanation`, `Pve_planner_queues_a_deterministic_move_then_attack_when_one_step_makes_the_target_legal`, `Pve_planner_prefers_advancing_cover_when_distance_is_equal`, `Pve_planner_medic_treats_the_most_injured_legal_ally`, `Pve_planner_uses_the_attack_profile_declared_by_a_marksman_unit`, `Pve_marksman_repositions_when_a_target_is_inside_its_minimum_range`, `Pve_hold_policy_keeps_an_objective_occupier_in_place_when_no_shot_is_legal`, `Pve_planner_scouts_toward_an_authored_objective_without_hidden_target_knowledge`, and `Graybox_four_v_four_pve_encounter_is_repeatable_across_multiple_rounds`.
